...
const action = ref()
const form = ref(null)
const formValues = ref({ numeroPedido: null, idFornecedor: null, dataPedido: null })

const formSchema = createPurchaseOrderSchema(() => action.value)
...
const view = async (purchaseOrder) => {
  ...
  form.value.setValues({ numeroPedido: purchaseOrder.numeroPedido, idFornecedor: purchaseOrder.fornecedor.id, dataPedido: formatDate(toDate(purchaseOrder.dataPedido)) })
  loadPurchaseOrderProducts(purchaseOrder.id)
}

defineExpose({ newOrder, view })
</script>
